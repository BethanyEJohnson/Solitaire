...
public class Pile {
	...
	/**
	 * The x and y locations of the pile. Though these are not final, they should
	 * NEVER change
	 */
	...
	/**
	 * Vertical displacement between cards in a pile. This only applies for cards in
	 * a MAIN_PILE or TEMP_PILE
	 */
	public static final int VERT_DISPL = 22;

	/**
	 * Horizontal displacement between cards. This only applies for cards in a
	 * DECK_PILE
	 */
	public static final int HORI_DISPL = 12;

	/**
	 * The top 3 cards in a deck pile. This is necessary because a maximum of three
	 * cards can be drawn in a deck pile. Also, if one card is removed, we must draw
	 * two, not three.
	 */
	private Card[] top3;

	/**
	 * No-arg constructor that sets the piles coordinates to (0,0) and sets the pile
	 * type to MAIN_PILE
	 */
	public Pile() {
		this(0, 0, TEMP_PILE);
	...
	/**
	 * Adds a pile to the pile and sets its location appropriately TODO: this might
	 * need to work for TEMP_PILE's as well but we'll see
	 * 
	...
	/**
	 * Returns the pile that has been clicked, if any. If only one card has been
	 * clicked, the pile will simply contain a single card
	 * 
	...
	/**
	 * Removes the card at index i from the deck or does nothing if the index is out
	 * of bounds
	 * 
	...
	/**
	 * Determines if the passed in card is on top of the pile (it's the most recent
	 ...
	 */
	public int getType() {
		// should be impossible for type to be anything except SUIT, MAIN, TEMP, or DECK
		// but just in case...
		return isValidPile(type) ? type : TEMP_PILE;
	}

	/**
	 * Sets the type to either MAIN, SUIT, or TEMP. If t is not any of these, it is
	 * set to TEMP
	 * 
	...
	/**
	 * Moves a TEMP_PILE. The method simply does nothing if the pile is not a
	 * TEMP_PILE. It also moves the x and y location of every card in the pile
	 * 
